import re
import sys
from isa import opcode_to_machine, machine_to_opcode, Registers, Opcode
    ...
    if token in labels:
        if offset in labels:
            code.append(
                generate_machine_code(
                    Opcode.LOAD, Registers.BR, None, addr=labels.index(offset)
                )
            )
            ...
            code.append(
                generate_machine_code(
                    Opcode.LOADIM, Registers.B, None, val=labels.index(token)
                )
            )
            ...
            code.append(
                generate_machine_code(
                    Opcode.LOAD, register, None, addr=labels.index(token) + int(offset)
                )
            )
        ...
        code.append(
            generate_machine_code(
                ...
                Registers.G6,
                None,
                addr=labels.index(token[1:]) + int(offset),
            )
        )
...
def count_expresion(expression: str, labels, line, line_number=0):
    ...
    tree = [
        [
            [
                [[i.strip().rstrip() for i in ili.split("&")] for ili in div.split("|")]
                for div in mul.split("/")
            ]
            for mul in plus.split("*")
        ]
        for plus in expression.split("+")
    ]
        ...
        for mul in pls:
            for j in range(len(mul)):
                ...
                for ili in div:
                    code.append(
                        generate_machine_code(
                            Opcode.LOADIM, Registers.G4, None, 0xFFFFF
                        )
                    )
                    for i in ili:
                        code += token_lable_or_val(
                            i, labels, Registers.G5, line_number, line
                        )
                        code.append(
                            generate_machine_code(
                                Opcode.AND, Registers.G4, Registers.G5
                            )
                        )
                    code.append(
                        generate_machine_code(Opcode.OR, Registers.G3, Registers.G4)
                    )
                if j > 0:
                    code.append(
                        generate_machine_code(Opcode.DIV, Registers.G6, Registers.G3)
                    )
                else:
                    code.append(
                        generate_machine_code(Opcode.MOV, Registers.G6, Registers.G3)
                    )
...
def translate(text):
    ...
    for line in lines:
        ...
        if symbols[0] == "int":
            ...
            code.append(
                generate_machine_code(
                    Opcode.STORE, Registers.G1, None, addr=len(labels) - 1
                )
            )
            ...
            code.append(
                generate_machine_code(Opcode.LOADIM, Registers.BR, None, lenght)
            )
            code.append(
                generate_machine_code(
                    Opcode.STORE, Registers.BR, None, addr=len(labels) - 1
                )
            )
            for i in range(lenght):
                ...
                if len(val) > i:
                    code.append(
                        generate_machine_code(Opcode.LOADIM, Registers.BR, None, val[i])
                    )
                else:
                    code.append(
                        generate_machine_code(Opcode.LOADIM, Registers.BR, None, 0)
                    )
                code.append(
                    generate_machine_code(
                        Opcode.STORE, Registers.BR, None, addr=len(labels) - 1
            ...
            if offset in labels:
                ...
                code.append(generate_machine_code(Opcode.LOADIM, Registers.A, None, 1))
                code.append(
                    generate_machine_code(Opcode.ADD, Registers.BR, Registers.A)
                )
                code.append(
                    generate_machine_code(
                        Opcode.LOADIM, Registers.B, None, val=labels.index(symbols[0])
                    )
                )
                code.append(
                    generate_machine_code(Opcode.ADD, Registers.BR, Registers.B)
                )
                code.append(
                    generate_machine_code(Opcode.STORE, Registers.BR, Registers.G1)
                )
            else:
                offset = int(offset)
                if type_of_labels[labels.index(symbols[0])] == "array":
                    offset += 1
                code.append(
                    generate_machine_code(
                        Opcode.STORE,
                        Registers.G1,
                        None,
                        addr=labels.index(symbols[0]) + offset,
                    )
                )
            ...
            left_expression, right_expression = (
                line.split("(")[1].split(")")[0].rstrip().strip().split(operand)
            )
            comeback = len(code)
            ...
            if operand == "==":
                code.append(
                    generate_machine_code(Opcode.JMPE, Registers.G1, Registers.A)
                )
            elif operand == "!=":
                code.append(
                    generate_machine_code(Opcode.JNE, Registers.G1, Registers.A)
                )
            else:
                code.append(
                    generate_machine_code(Opcode.JMPL, Registers.G1, Registers.A)
                )
            ...
            if len(symbols) != 4:
                raise ValueError(f"{line}\nIncorrect expresion on line {line_num}")
            to_print = symbols[2]
            if to_print not in labels:
                raise ValueError(f"{line}\nNo such string declared line {line_num}")
            if type_of_labels[labels.index(to_print)] == "int":
                # Умеет печатать только цифры!!
                code.append(
                    generate_machine_code(
                        Opcode.LOAD, Registers.G1, None, addr=labels.index(to_print)
                    )
                )
                code.append(
                    generate_machine_code(Opcode.STORE, Registers.G1, None, addr=1)
                )
                ...
                code.append(
                    generate_machine_code(
                        Opcode.LOADIM, Registers.G2, None, val=labels.index(to_print)
                    )
                )
                code.append(
                    generate_machine_code(Opcode.LOADIM, Registers.G3, None, val=1)
                )
                # Счётчик в BR
                code.append(
                    generate_machine_code(Opcode.LOADIM, Registers.BR, None, val=0)
                )
                # while (BR < G1)
                code.append(
                    generate_machine_code(Opcode.JMPL, Registers.BR, Registers.G1)
                )
                code.append(generate_machine_code(Opcode.JMP, addr=len(code) + 6))
                # BR++
                code.append(
                    generate_machine_code(Opcode.ADD, Registers.BR, Registers.G3)
                )
                code.append(
                    generate_machine_code(Opcode.ADD, Registers.G2, Registers.G3)
                )
                code.append(
                    generate_machine_code(Opcode.LOAD, Registers.G2, Registers.G4)
                )
                code.append(
                    generate_machine_code(Opcode.STORE, Registers.G4, None, addr=1)
                )
                code.append(generate_machine_code(Opcode.JMP, addr=len(code) - 6))
            elif type_of_labels[labels.index(to_print)] == "I":
                code.append(
                    generate_machine_code(Opcode.LOAD, Registers.B, None, addr=0)
                )
                code.append(
                    generate_machine_code(Opcode.STORE, Registers.B, None, addr=1)
                )
            else:
                raise ValueError(
            ...
            code[0] = generate_machine_code(
                Opcode.LOADIM, Registers.INT, None, val=len(code)
            )

            # Сохраняем состояние
            for i in range(10):
                code.append(
                    generate_machine_code(Opcode.STORE, Registers(i), None, addr=i + 2)
                )
    ...
    if int_func_present:
        ...
        for i in range(10):
            code.append(
                generate_machine_code(Opcode.LOAD, Registers(i), None, addr=i + 2)
            )
...
def code_to_bin(code, bin_filename, hex_filename):
    ...
    print(
        f"Saved {len(byte_data)} bytes to {bin_filename} (binary) and {hex_filename} (hex)"
    )
...
if __name__ == "__main__":
    ...
    with open(out_file + ".txt", "w") as res:
        for c in range(len(code)):
            res.write(
                f"{c}. {code[c]} - {machine_to_opcode[int(code[c][0], 16)].name}\n"
            )
