...
def alu(alur,alul,code):
    ...
    if (type(alur) == str): alur = int(alur)
    if (type(alul) == str): alul = int(alul)

    if (alur == 1048575): alur = -1
    if (alul == 1048575): alul = -1
...
def decode_instruction(instruction:int):
    ...
    if mode_code[0] == 1:
        ...
        if op_name == Opcode.JMP: 
            ...
            if (op_name != Opcode.JMPL and op_name != Opcode.JMPE and op_name != Opcode.JNE): 
                mach_cicles['EX']['res_latch']  = 1
                mach_cicles['WB']['regs_latchs'] = [r1]
            else:
